...
export async function POST(req: Request) {
  try {
    ...
    // 1. Booking.com RapidAPI
    const fetchBooking = async () => {
      try {
        if (!process.env.BOOKING_RAPIDAPI_KEY) return [];
        const destResp = await fetch(`https://${process.env.BOOKING_RAPIDAPI_HOST}/api/v1/hotels/searchDestination?query=${encodeURIComponent(resolvedLocation)}`, {
          headers: { 'x-rapidapi-key': process.env.BOOKING_RAPIDAPI_KEY, 'x-rapidapi-host': process.env.BOOKING_RAPIDAPI_HOST! }
        });
        const destData = await destResp.json();
        if (destData?.data?.[0]?.dest_id) {
          const hotelResp = await fetch(`https://${process.env.BOOKING_RAPIDAPI_HOST}/api/v1/hotels/searchHotels?dest_id=${destData.data[0].dest_id}&search_type=${destData.data[0].search_type}&arrival_date=${checkIn}&departure_date=${checkOut}&adults=${guests}&currency_code=INR`, {
            headers: { 'x-rapidapi-key': process.env.BOOKING_RAPIDAPI_KEY, 'x-rapidapi-host': process.env.BOOKING_RAPIDAPI_HOST! }
          });
          const hotelData = await hotelResp.json();
          return (hotelData?.data?.hotels || []).slice(0, HOTELS_PER_OTA_PAGE).map((h: any) => ({
            id: `bk-${h.property?.id || Math.random()}`,
            name: h.property?.name || 'Booking Premium',
            area: 'City Center',
            stars: h.property?.reviewScoreWord ? 5 : 4,
            rating: h.property?.reviewScore || 4.5,
            reviews: h.property?.reviewCount || 120,
            price: `₹${Math.round(h.property?.priceBreakdown?.grossPrice?.value || 15000).toLocaleString('en-IN')}`,
            priceNum: Math.round(h.property?.priceBreakdown?.grossPrice?.value || 15000),
            perNight: true,
      ...
      } catch (e) { console.warn("[Booking.com API] Failed", e); }
      return [];
    };
