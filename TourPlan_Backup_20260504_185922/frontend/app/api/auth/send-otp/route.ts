...
/** 4-digit numeric OTP only (matches UI maxLength). */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const from =
      process.env.RESEND_FROM_EMAIL?.trim() || 'TourPlan <onboarding@resend.dev>';
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [target],
        subject: 'Your TourPlan verification code',
        html: `
    ...
    return NextResponse.json({
      ok: true,
      message: 'We emailed you a 4-digit verification code.',
      cooldownSeconds: Math.ceil(COOLDOWN_MS / 1000),
      otpDigits: 4,
      otpChannel: 'custom',
    });
