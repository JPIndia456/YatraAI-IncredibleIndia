...
import {
  ...
  Calendar, Navigation, CheckCircle2, Hotel, Car, Phone, Ship
} from 'lucide-react';
...
// ── Typing cursor blink ───────────────────────────────────────────────────────
const Cursor = () => (
  <span className="inline-block w-[2px] h-[1em] bg-blue-600 ml-0.5 align-middle animate-cursor-blink" />
);
        ...
        return (
          <motion.button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold border transition-all
              ${ isSelected
                ? 'bg-blue-600 border-blue-600 text-black shadow-lg shadow-blue-600/30'
                : 'bg-zinc-800 border-zinc-700 text-blue-600 hover:bg-blue-600/15 hover:border-blue-600/50'
              }`}
          >
...
const DiscoveryCard = memo(function DiscoveryCard({ data }: { data: Record<string, string> }) {
  ...
  const colorClass = (type === 'air' || type === 'flight') ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 
                     (type === 'rail' || type === 'train') ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
                     (type === 'stay' || type === 'hotel') ? 'text-violet-400 bg-violet-500/10 border-violet-500/20' :
  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 p-3 opacity-5 group-hover:opacity-10 transition-opacity">
        <Sparkles className="w-12 h-12 text-[#003366]" />
      </div>
      ...
      <div className="flex justify-between items-start gap-3 relative z-10">
        <div className="flex gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-black text-[#003366] uppercase tracking-tight truncate">{name}</p>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider truncate mt-0.5">{detail}</p>
          </div>
        </div>
        <div className="text-right shrink-0">
          <div className="text-sm font-black text-[#003366] italic">{price}</div>
      ...
      <button
        onClick={() => {
          ...
          if (type === 'air' || type === 'flight' || type === 'rail' || type === 'train') setMixPicks({ transport: tierItem });
          else if (type === 'stay' || type === 'hotel') setMixPicks({ hotel: tierItem });
          else if (type === 'mobility') setMixPicks({ local: tierItem });
          
          toast.success(`Added ${name} to your Odyssey plan!`);
        }}
        className="w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all bg-white text-zinc-950 hover:bg-cyan-400 shadow-xl shadow-white/5 active:scale-95"
      >
        Select & Add to Plan →
      </button>
      ...
      {data.link && (
        <a 
          ...
          className="flex items-center justify-center gap-1.5 text-[8px] font-black text-slate-500 hover:text-[#003366] uppercase tracking-widest transition-all mt-1"
        >
...
const SelectionCard = memo(function SelectionCard({ type, index }: { type: string; index: number }) {
  ...
  const colorClass = type === 'air' ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 
                     type === 'rail' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
                     type === 'stay' ? 'text-violet-400 bg-violet-500/10 border-violet-500/20' :
                     'text-blue-600 bg-blue-600/10 border-blue-600/20';
    ...
    >
      <div className="flex justify-between items-start gap-3">
        <div className="flex gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-black text-real-white uppercase tracking-tight truncate">{displayName}</p>
            <p className="text-[9px] text-black/40 font-bold uppercase tracking-wider truncate">{displayDetail}</p>
          </div>
        </div>
        <div className="text-right shrink-0">
          <div className="text-xs font-black text-[#003366] italic">{item.price}</div>
      ...
      <button
        onClick={() => {
          ...
          if (type === 'air' || type === 'rail') setMixPicks({ transport: tierItem });
          else if (type === 'stay') setMixPicks({ hotel: tierItem });
          else if (type === 'mobility') setMixPicks({ local: tierItem });
        ...
        className="w-full py-2 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] transition-all border border-accent-amber/20 bg-accent-amber text-black shadow-lg shadow-accent-amber/20 hover:brightness-110 active:scale-95"
      >
        Select & Add to Plan
      </button>
    </motion.div>
...
const MessageBubble = memo(function MessageBubble({
  ...
  return (
    ...
    >
      {/* Avatar */}
      {!isUser && (
        <div className="w-6 h-6 rounded-xl bg-gradient-to-br from-accent-amber to-orange-500 flex items-center justify-center shrink-0 mt-1 shadow-[0_0_10px_rgba(34,211,238,0.3)]">
          <Sparkles className="w-3 h-3 text-[#003366]" />
        </div>
      )}
      <div className={`max-w-[86%] ${ !isUser ? 'w-full' : '' }`}>
        <div
          className={`px-3 py-2 rounded-xl text-[12px] leading-relaxed font-medium whitespace-pre-wrap
            ${isUser
              ? 'bg-[#0047AB] text-real-white rounded-tr-sm shadow-md'
              : 'bg-black/5 border border-black/10 text-black/90 rounded-tl-sm'
            }`}
        >
          {/* Render content, but hide [SELECT:...] and [UPDATE:...] tags from the UI */}
          {displayContent.replace(/\[UPDATE:.*?\]/g, '').replace(/\[SELECT:.*?\]/g, '').replace(/\[DISCOVERY:.*?\]/g, '')}
          {msg.isStreaming && <Cursor />}
          {isLong && (
            <button 
              onClick={() => setIsExpanded(!isExpanded)}
              className="block mt-2 text-[10px] text-blue-600 font-bold uppercase tracking-widest hover:text-blue-600 transition-colors"
            >
  ...
  <div className="space-y-4 pt-2">
    {plannerStage === 'booking' && (
      <div className="bg-slate-50 border border-black/5 rounded-2xl p-4 space-y-3 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-accent-amber/10 flex items-center justify-center border border-orange-500/20 shadow-inner">
            <Send className="w-5 h-5 text-orange-600" />
          </div>
          <div className="text-left">
            <p className="text-micro font-black text-orange-600 uppercase tracking-widest">Active Safety</p>
            <p className="text-[10px] text-[#003366] font-bold tracking-tight">Telegram Connected</p>
          </div>
        </div>
    ...
    {plannerStage === 'selection' && (
      <div className="space-y-4">
        <div className="bg-blue-500/10 border border-indigo-500/20 rounded-2xl p-4 space-y-3 shadow-xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-accent-amber flex items-center justify-center shadow-lg shadow-indigo-500/20">
              <BrainCircuit className="w-5 h-5 text-zinc-950" />
            </div>
            <div className="text-left">
              <p className="text-micro font-black text-orange-600 uppercase tracking-widest">Master Control</p>
              <p className="text-[10px] text-[#003366] font-bold tracking-tight">Modify & Re-Craft</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
             <button onClick={() => sendMessage("I want to increase my budget. What are the premium options?")} className="py-2 bg-slate-50 border border-slate-200 rounded-xl text-[8px] font-black text-[#003366] uppercase tracking-widest">Increase Budget</button>
             <button onClick={() => sendMessage("I want to reduce my budget. Show me more economical choices.")} className="py-2 bg-slate-50 border border-slate-200 rounded-xl text-[8px] font-black text-[#003366] uppercase tracking-widest">Reduce Budget</button>
          </div>
        </div>
        ...
        <button 
          ...
          className="w-full py-4 bg-gradient-to-r from-accent-amber to-orange-600 rounded-2xl border border-accent-amber/20 flex items-center justify-center gap-3 shadow-2xl group transition-all active:scale-95"
        >
          ...
          <div className="text-left">
             <p className="text-[10px] font-black text-[#003366] uppercase tracking-widest leading-none">Select/Choose</p>
          ...
          <div className="bg-slate-50 border border-black/5 rounded-2xl p-4 space-y-3 shadow-xl">
             ...
             <div className="space-y-2">
                {mixPicks.transport && (
                  <div className="flex items-center justify-between group/pick">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 rounded-lg bg-zinc-800"><Plane className="w-3 h-3 text-blue-600" /></div>
                      <div className="text-left">
                        <p className="text-[9px] font-black text-[#003366] uppercase leading-none">{mixPicks.transport.name}</p>
                ...
                {mixPicks.hotel && (
                  <div className="flex items-center justify-between group/pick">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 rounded-lg bg-zinc-800"><Hotel className="w-3 h-3 text-emerald-400" /></div>
                      <div className="text-left">
                        <p className="text-[9px] font-black text-[#003366] uppercase leading-none">{mixPicks.hotel.name}</p>
                ...
                {mixPicks.local && (
                  <div className="flex items-center justify-between group/pick">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 rounded-lg bg-zinc-800"><Car className="w-3 h-3 text-orange-600" /></div>
                      <div className="text-left">
                        <p className="text-[9px] font-black text-[#003366] uppercase leading-none">{mixPicks.local.name}</p>
        ...
        {(flightOptions.length > 0 || trainOptions.length > 0 || ferryOptions.length > 0 || hotelOptions.length > 0 || mobilityOptions.length > 0) && (
          <div className="bg-slate-50 border border-black/5 rounded-2xl p-4 space-y-4 shadow-xl">
            <p className="text-[9px] font-black text-blue-600 uppercase tracking-[0.2em]">AI Quick Selectors</p>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {flightOptions.map((f: any, idx: number) => {
                    ...
                    return (
                      <label key={`ai-flight-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer ${selected ? 'border-blue-600/40 bg-blue-600/10' : 'border-black/5 bg-white/80'}`}>
                        <input
                          type="radio"
                        ...
                        />
                        <div className="min-w-0">
                          <p className="text-[9px] font-black text-[#003366] uppercase truncate">{name}</p>
                          <p className="text-[8px] text-black/40 font-bold uppercase truncate">{detail}</p>
                        </div>
                        <span className="ml-auto text-[9px] font-black text-blue-600">{f.price || '—'}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {hotelOptions.map((h: any, idx: number) => {
                    ...
                    return (
                      <label key={`ai-hotel-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer ${selected ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-black/5 bg-white/80'}`}>
                        <input
                          type="radio"
                        ...
                        <span className="ml-auto text-[9px] font-black text-emerald-400">{h.price || '—'}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {trainOptions.map((t: any, idx: number) => {
                    ...
                    return (
                      <label key={`ai-train-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer ${selected ? 'border-blue-600/40 bg-blue-600/10' : 'border-black/5 bg-white/80'}`}>
                        <input
                          type="radio"
                        ...
                        <span className="ml-auto text-[9px] font-black text-blue-600">{t.price || '—'}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {ferryOptions.map((f: any, idx: number) => {
                    ...
                    return (
                      <label key={`ai-ferry-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer ${selected ? 'border-teal-500/40 bg-teal-500/10' : 'border-black/5 bg-white/80'}`}>
                        <input
                          type="radio"
                        ...
                        <span className="ml-auto text-[9px] font-black text-teal-400">{f.price || '—'}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                  {mobilityOptions.map((m: any, idx: number) => {
                    ...
                    return (
                      <label key={`ai-mobility-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer ${selected ? 'border-violet-500/40 bg-violet-500/10' : 'border-black/5 bg-white/80'}`}>
                        <input
                          type="radio"
                        ...
                        <span className="ml-auto text-[9px] font-black text-violet-400">{m.price || '—'}</span>
                      </label>
                    );
                  })}
                </div>
    ...
    {(activeItinerary || (tiers && tiers.length > 0)) && (
      <div className="bg-blue-600/5 border border-blue-600/15 rounded-2xl p-4 space-y-2 text-left">
        ...
        {activeItinerary ? (
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm font-black text-blue-600 mt-2">{activeItinerary.total}</p>
            </div>
            
            <div className="border-t border-blue-600/20 pt-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button 
                  ...
                  className="flex flex-col items-center justify-center gap-1 py-2 px-1 bg-slate-50 border border-black/5 rounded-lg hover:border-blue-600/50 hover:bg-blue-600/10 transition-all"
                >
            ...
            <button onClick={() => { sendMessage(`Tell me more about my ${activeItinerary.tierLabel} trip plan from ${activeItinerary.from} to ${activeItinerary.to} costing ${activeItinerary.total}`); }}
              className="w-full py-2 bg-blue-600/10 border border-blue-600/20 text-blue-600 text-[10px] font-black rounded-xl hover:bg-blue-600/20 transition-all">
          ...
          <div className="space-y-1">
            ...
            <button 
              ...
              className="mt-1 w-full py-1.5 bg-blue-600/10 border border-blue-600/20 text-blue-600 text-[9px] font-black rounded-xl hover:bg-blue-600/20 transition-all"
            >
    ...
    <div className="bg-blue-500/5 border border-blue-500/15 rounded-2xl p-4 space-y-2 text-left">
      <div className="flex items-center gap-2">
        <ShieldCheck className="w-3.5 h-3.5 text-blue-400" />
        <span className="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Caring AI Agent</span>
      </div>
    ...
    <div className="bg-[#fdf8f3] border border-orange-200/50 rounded-2xl p-4 space-y-3 text-left shadow-lg shadow-orange-900/5">
      <div className="flex items-center justify-between">
        ...
        <motion.button
          ...
          className="w-10 h-10 bg-sky-400 rounded-full flex items-center justify-center shadow-lg shadow-sky-400/30 relative group shrink-0"
        >
          <Mic className="w-5 h-5 text-[#003366] group-hover:scale-110 transition-transform" />
          <div className="absolute inset-0 rounded-full border-2 border-sky-400 opacity-20 animate-ping" />
        </motion.button>
      </div>
    ...
    <div className="shrink-0 px-4 py-3 bg-slate-1000 border-b border-slate-200 flex gap-2 overflow-x-auto scrollbar-hide">
      {dynamicPrompts.map(({ icon: Icon, label, prompt }: any) => (
        <button
          ...
          disabled={isStreaming}
          className={`flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-black/5 rounded-full whitespace-nowrap hover:border-blue-500/50 transition-all group disabled:opacity-50 disabled:cursor-not-allowed`}
        >
          <Icon className="w-3 h-3 text-blue-400" />
          <span className="text-[8px] font-black uppercase tracking-widest text-black/40 group-hover:text-[#003366]">{label}</span>
        </button>
      ))}
    </div>
...
export default function AIBrain({ context }: { context?: Record<string, unknown> }) {
  ...
  const { destination, startDate, endDate, origin, isOnboarded } = useTripStore();
  const { activeItinerary, tiers, searchData, plannerStage, mixPicks, setPlannerStage } = useTripPlannerStore();
  ...
  const startWizard: () => void = useCallback(() => {
    ...
    addMessage({ role: 'assistant', content: `Namaste Traveller` });
    
    setTimeout(() => {
      injectWizardQuestion(schema[0].id, initialData);
  ...
  const handleWizardAnswer = useCallback((text: string) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setWizardStep(nextId === '__done__' ? 'done' : nextId);
  ...
  const hasInitiatedStartRef = useRef(false);
  useEffect(() => {
    const currentTrip = useTripStore.getState();
    const currentTour = useTourGuideStore.getState();
    const allCoreFilled = (currentTrip.destination || currentTour.destination) && 
                         (currentTrip.origin || currentTour.from_city) && 
                         (currentTrip.startDate || currentTour.departure_date);

    if (isOpen && messages.length === 0 && wizardSchema.length > 0 && !hasInitiatedStartRef.current) {
      hasInitiatedStartRef.current = true;
      
      // If form is already partially filled, don't force the wizard.
      if (allCoreFilled) {
        useAIBrainStore.getState().setWizardMode(false);
        addMessage({ role: 'assistant', content: `Namaste Traveller! 🇮🇳 I see your plan is already taking shape. How can I help you perfect it?` });
        return;
      }
      
      // Proactive Wizard Start: Ask the first question immediately
      useAIBrainStore.getState().setWizardMode(true);
      const firstStep = wizardSchema[0];
      setWizardStep(firstStep.id);
      injectWizardQuestionRef.current?.(firstStep.id, {});
    }
    // Reset if user manually clears or wizard is toggled off
    if (!isOpen) {
      hasInitiatedStartRef.current = false;
    }
  }, [isWizardMode, isOpen, wizardSchema.length, messages.length]);
  ...
  const dynamicPrompts = useMemo(() => [
    ...
    },
    {
      icon: Thermometer,
      label: 'Weather',
      prompt: destination ? `What is the weather like in ${destination} right now?` : 'What is the weather like at my destination?'
    },
    {
      icon: Wallet,
      label: 'Budget tips',
  ...
  useEffect(() => {
    async function loadHistory() {
      if (!user?.id) return;
      const { data, error } = await supabase.from('tourplan_ai_chat_history').select('role, content, created_at').eq('user_id', user.id).order('created_at', { ascending: true }).limit(20);
      if (error) return;
  ...
  */
  ...
  const clearChatHistory = async () => {
    clearHistory();
    if (user?.id) {
      await supabase.from('tourplan_ai_chat_history').delete().eq('user_id', user.id);
    }
  };
          ...
          >
            ...
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none opacity-20">
              <motion.div 
                ...
                className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-blue-600/20 rounded-full blur-[80px]" 
              />
              <motion.div 
                animate={{ 
                  scale: [1.2, 1, 1.2],
              ...
              <motion.div 
                ...
                className="absolute top-[20%] right-[10%] w-[40%] h-[40%] bg-blue-600/15 rounded-full blur-[60px]" 
              />
            </div>
            ...
            <div className="shrink-0 flex items-center gap-3 px-4 h-14 border-b border-slate-200 bg-white/[0.02]">
              ...
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-600 to-blue-500 flex items-center justify-center shrink-0 shadow-lg shadow-blue-600/20">
                  <Sparkles className="text-[#003366]" size={15} />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${isStreaming || isListening || isSpeaking ? 'bg-amber-400 animate-pulse' : 'bg-emerald-500'}`} />
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <div className="relative group/lang">
                  <button
                    className="p-1.5 text-black/40 hover:bg-zinc-800 hover:text-black/70 rounded-lg transition-colors flex items-center gap-1"
                  ...
                  <div className="absolute right-0 bottom-full mb-1 w-48 max-h-60 overflow-y-auto bg-slate-50 border border-black/5 rounded-xl py-1 shadow-2xl opacity-0 invisible group-hover/lang:opacity-100 group-hover/lang:visible transition-all z-50 flex flex-col">
                    {SUPPORTED_LANGUAGES.map(lang => (
                      <button
                        ...
                        className={`flex items-center justify-between w-full text-left px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest hover:bg-zinc-800 transition-colors ${language === lang.code ? 'text-emerald-400 bg-emerald-500/10' : 'text-black/50'}`}
                      >
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all ${audioFeedback ? 'text-blue-600 bg-blue-600/10 border border-blue-600/20' : 'text-black/40 hover:bg-zinc-800 hover:text-black/70'}`}
                ...
                {messages.length > 0 && (
                  <button
                    ...
                    className="p-1.5 text-black/40 hover:bg-zinc-800 hover:text-black/50 rounded-lg transition-colors"
                ...
                <button
                  ...
                  className="p-1.5 text-black/40 hover:bg-zinc-800 hover:text-[#003366] rounded-lg transition-colors"
                  title="Close"
                >
                  <X className="w-4 h-4" />
                </button>
            ...
            {wizardStep && wizardStep !== 'done' && (() => {
              const STEPS = ['destination', 'origin', 'startDate', 'endDate', 'budget', 'adults'];
              const LABELS = ['Where', 'From', 'Depart', 'Return', 'Budget', 'Party'];
              const ICONS = ['🗺️', '🏙️', '📅', '🔙', '💰', '👥'];
              const currentIdx = STEPS.indexOf(wizardStep);
              return (
                <div className="shrink-0 px-4 py-2.5 bg-white border-b border-black/5">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[8px] font-black uppercase tracking-widest text-blue-600">Trip Setup</span>
                    <span className="text-[8px] font-bold text-black/30">{currentIdx + 1} / {STEPS.length}</span>
                  </div>
                    ...
                    {STEPS.map((s, i) => (
                      <div key={s} className="flex-1 flex flex-col items-center gap-0.5">
                        <div className={`h-1 w-full rounded-full transition-all duration-500 ${
                          ...
                          i === currentIdx ? 'bg-blue-600 shadow-[0_0_6px_rgba(34,211,238,0.6)] animate-pulse' :
                          'bg-zinc-800'
                        }`} />
                        <span className={`text-[7px] font-bold transition-colors ${
                          i === currentIdx ? 'text-blue-600' : i < currentIdx ? 'text-black/40' : 'text-black/20'
                        }`}>{ICONS[i]} {LABELS[i]}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {destination && !wizardStep && (
              <div className="shrink-0 flex items-center gap-2 px-4 py-2 bg-blue-600/5 border-b border-blue-600/10">
                <MapPin className="w-3 h-3 text-blue-600 shrink-0" />
                <span className="text-[10px] font-bold text-[#003366] truncate">{destination}</span>
                <span className="text-[9px] text-black/30 shrink-0">{startDate} → {endDate}</span>
                <span className="ml-auto text-[8px] font-black text-blue-600 bg-blue-600/10 px-2 py-0.5 rounded-full uppercase tracking-widest border border-blue-600/15 shrink-0">Active Trip</span>
              </div>
            )}
            ...
            <div className="shrink-0 px-3 py-2 border-b border-black/5 bg-white/30">
              <div className="bg-black/5 border border-black/5 rounded-xl p-1 flex gap-1 overflow-x-auto no-scrollbar">
                {syncedStages.map((st) => {
                  ...
                  const isAccessible = targetIdx <= currentIdx || (st.id === 'results' && searchData.hotels.length > 0);

                  return (
                    <button
                      key={st.id}
                      disabled={!isAccessible}
                      onClick={() => isAccessible && setPlannerStage(st.id)}
                      className={`min-w-[64px] px-2 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${
                        isActive
                          ? 'bg-accent-amber text-black shadow-[0_4px_12px_rgba(255,191,0,0.3)]'
                          : isAccessible 
                  ...
                  <button
                    ...
                    className={`relative w-14 h-14 rounded-full transition-all flex items-center justify-center
                      ${isListening || isServerVoiceRecording
                        ? 'bg-sky-400 shadow-[0_0_28px_rgba(56,189,248,0.65)] scale-110 animate-pulse'
                        : isSpeaking
                        ? 'bg-sky-500 shadow-[0_0_28px_rgba(56,189,248,0.65)] scale-110 animate-pulse'
                        : isStreaming
                        ? 'bg-black/5 animate-pulse'
                        : 'bg-black/5 hover:bg-black/10 border border-black/10'}`}
                  >
                    <Mic className="w-6 h-6 text-[#003366]" />
                  </button>
                  <span
                  ...
                  {!VOICE_FORCE_SERVER_STT &&
                    ...
                    serverVoiceFallbackUi && (
                      <button
                        ...
                        className="text-[9px] font-bold text-blue-600/90 hover:text-blue-600 uppercase tracking-wider"
                      >
                ...
                <div className="flex items-end gap-2">
                  ...
                  {selectedImage ? (
                    <div className="relative w-10 h-10 shrink-0">
                      ...
                      <button onClick={() => setSelectedImage(null)} className="absolute -top-1.5 -right-1.5 bg-red-500 w-4 h-4 rounded-full flex items-center justify-center shadow">
                        <X size={8} className="text-[#003366]" />
                      </button>
                    </div>
                  ) : (
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className="p-2 text-black/30 hover:text-black/70 hover:bg-zinc-800 rounded-lg transition-colors shrink-0"
                      title="Attach image"
                    >
                  ...
                  <div className="flex-1">
                    <textarea
                      ...
                      disabled={isStreaming}
                      rows={1}
                      className="w-full bg-black/5 border border-black/5 rounded-xl px-3 py-2 text-[13px] text-black placeholder-black/40 outline-none focus:border-zinc-600 transition-colors resize-none disabled:opacity-50 leading-relaxed"
                  ...
                  {isStreaming ? (
                    <button
                      onClick={handleStop}
                      className="p-2.5 bg-white border border-black/10 text-black/80 rounded-xl hover:bg-zinc-700 transition-all shrink-0"
                      title="Stop"
                    >
                      <X className="w-4 h-4" />
                    </button>
                    ...
                    <button
                      id="tourplan-ai-send-btn"
                      onClick={() => sendMessage(inputValue)}
                      disabled={!inputValue.trim() || isStreaming}
                      className="p-2.5 bg-accent-amber text-black rounded-2xl disabled:opacity-25 hover:brightness-110 active:scale-90 transition-all shrink-0 shadow-[0_4px_12px_rgba(255,191,0,0.3)]"
                      title="Send"
                    >
              ...
              <div className="flex items-center justify-between pt-1">
                <div className="flex items-center gap-0.5 bg-black/5 border border-black/10 rounded-xl p-1">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all ${
                      useVoiceMode ? 'bg-sky-400 text-[#003366]' : 'text-black/40 hover:text-black/80'
                    }`}
                  >
        ...
        <AnimatePresence>
          {!isOpen && (
            <motion.button
              ...
              className="relative w-16 h-16 rounded-full flex items-center justify-center text-[#003366] shadow-[0_12px_40px_rgba(255,191,0,0.4)] border border-slate-300 group overflow-hidden bg-black"
            >
