...
export default function ShellToolbar() {
  ...
  const navLinks = [
    { label: t('planner'), icon: Map, href: '/planner', color: 'from-accent-amber to-orange-600' },
    { label: t('my_trips'), icon: ClipboardList, href: '/my-trip', color: 'from-orange-600 to-amber-500' },
    { label: t('profile'), icon: User, href: '/profile', color: 'from-zinc-600 to-zinc-900' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-4">
        ...
        >
          <div className="relative">
            <div className="absolute inset-0 bg-accent-amber blur-xl opacity-10 group-hover:opacity-30 transition-opacity" />
            <div className="w-11 h-11 bg-gradient-to-br from-accent-amber to-orange-600 rounded-2xl flex items-center justify-center shadow-lg shadow-accent-amber/20 relative z-10">
               <PlaneTakeoff className="text-white w-6 h-6" />
            </div>
          </div>
          <div className="hidden sm:block">
            <div className="text-xl font-semibold tracking-tight text-zinc-900 group-hover:text-accent-amber transition-colors leading-none">
              TOUR<span className="text-accent-amber font-bold">PLAN 🇮🇳</span>
            </div>
            <div className="text-[10px] text-orange-600 opacity-70 group-hover:opacity-100 transition-opacity mt-1 uppercase tracking-wide font-medium">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
              ...
              {SUPPORTED_LANGUAGES.map((lang) => (
                <button
                  ...
                  className={`flex items-center justify-between w-full text-left px-4 py-2 text-xs font-semibold hover:bg-zinc-50 transition-colors ${
                    language === lang.code ? 'bg-orange-500/10 text-orange-600' : 'text-zinc-500 hover:text-zinc-700'
                  }`}
                >
                  <span>{lang.name}</span>
          ...
          >
            {authLoading ? (
              ...
              <div className="w-9 h-9 bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl flex items-center justify-center text-xs font-bold text-white group-hover:scale-110 transition-transform overflow-hidden">
                {avatarUrl ? (
            ...
            <div className="px-4 py-4 space-y-2">
              ...
              {user && (
                <div className="flex items-center gap-3 p-4 bg-zinc-50 rounded-2xl border border-zinc-100 mb-4">
                  <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl flex items-center justify-center text-xs font-bold text-white overflow-hidden shrink-0">
                    {avatarUrl ? (
                      <img src={avatarUrl} alt={userName} className="w-full h-full object-cover" />
                    ) : (
                      initials || '?'
              ...
              {user ? (
                ...
                <button
                  ...
                  className="w-full py-3 bg-orange-500/5 border border-orange-500/10 text-orange-600 rounded-2xl text-sm font-bold"
                >
                  {t('sign_in')}
                </button>
              )}
