...
function EstimateTabs({
  fNum, tNum, hNum, taxiNum, isTaxiGrey, currentPax, nights, currentRooms, fetchedRooms, active, setActive
}: any) {
  ...
  const tabs = [];
  if (fNum > 0) tabs.push({ id: 'air', label: 'Air', total: gFlight, icon: Plane, color: 'text-blue-500', bg: 'bg-blue-500/10' });
  if (tNum > 0) tabs.push({ id: 'rail', label: 'Rail', total: gTrain, icon: Train, color: 'text-amber-800', bg: 'bg-amber-800/10' });
  if (fNum > 0 && tNum > 0) tabs.push({ id: 'mix', label: 'Mix', total: gMix, icon: Sparkles, color: 'text-orange-500', bg: 'bg-orange-500/10' });
  ...
  const lineItems = active === 'mix'
    ? [
      { icon: Plane, label: 'Flight (1-way)', val: Math.round(fTotal / 2), color: 'text-blue-500' },
      { icon: Train, label: 'Train (1-way)', val: Math.round(tTotal / 2), color: 'text-amber-800' },
      { icon: Hotel, label: `Stay (${nights}N)`, val: hTotal, color: 'text-emerald-500' },
      { icon: Car, label: 'Taxi', val: taxiNum, color: 'text-blue-600', grey: isTaxiGrey },
    ]
    : [
      { icon: active === 'air' ? Plane : Train, label: active === 'air' ? `Flight (${currentPax} pax)` : `Train (${currentPax} pax)`, val: active === 'air' ? fTotal : tTotal, color: active === 'air' ? 'text-blue-500' : 'text-amber-800' },
      { icon: Hotel, label: `Stay (${nights}N)`, val: hTotal, color: 'text-emerald-500' },
      { icon: Car, label: 'Taxi', val: taxiNum, color: 'text-blue-600', grey: isTaxiGrey },
    ];

  return (
    <div className="bg-blue-600/8 border border-blue-600/15 rounded-xl p-3 space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Grand Total Estimate</p>
        <div className="flex bg-black/30 rounded-lg p-0.5 gap-0.5">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={e => { e.stopPropagation(); setActive(tab.id); }}
              className={`px-2 py-0.5 rounded-md text-[9px] font-black transition-all ${active === tab.id ? `${tab.bg} ${tab.color}` : 'text-slate-400 hover:text-white'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      <p className="text-2xl font-black text-[#003366] leading-none tracking-tighter italic">{inr(cur.total)}</p>
      <div className="space-y-1 pt-1 border-t border-slate-200/50">
        {lineItems.map((item, idx) => (
          <div key={idx} className={`flex items-center justify-between text-xs ${item.grey ? 'opacity-30 line-through' : ''}`}>
            ...
            <span className="font-semibold text-[#003366]">{inr(item.val)}</span>
          </div>
        ))}
      </div>
    </div>
...
function SuggestionCard({ s, i, inputs, fetchedInputs, setInputs, onConfirm, onAskAI }: SuggestionCardProps) {
  ...
  const [activeTab, setActiveTab] = useState(fNum > 0 ? 'air' : 'rail');

  const formatPrice = (str?: string | number | null, pax = 1) => {
    const n = getNum(str);
    if (!n || str === 'N/A') return '—';
    const line = n * pax;
  ...
  const transportItems = [
    { id: 'air', icon: Plane, label: 'Flight', val: s.flight_cost, pax: currentPax, color: 'blue', detail: s.flight_name || s.nearest_airport },
    { id: 'rail', icon: Train, label: 'Train', val: s.train_cost, pax: currentPax, color: 'cyan', detail: s.train_name || s.nearest_railway },
    { id: 'taxi', icon: Car, label: 'Taxi', val: s.taxi_cost, pax: 1, color: 'emerald', detail: 'Round-trip est.' },
    { id: 'hotel', icon: Hotel, label: 'Hotel/night', val: s.hotel_per_night, pax: 1, color: 'purple', detail: s.hotel_name || 'Per night est.' },
  ].filter(item => {
    if (item.label === 'Flight' && (!s.flight_cost || s.flight_cost === 'N/A') && (!s.train_cost || s.train_cost === 'N/A')) return false;
    return true;
  });
  ...
  return (
    ...
    >
      {/* Hover glow */}
      <div className="absolute -inset-px bg-gradient-to-r from-blue-600/15 to-blue-600/15 rounded-2xl blur-sm opacity-0 group-hover:opacity-100 transition-all duration-500 pointer-events-none" />
      ...
      <div className="relative glass-panel p-4 md:p-5 space-y-3 rounded-xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h3 className="text-lg font-black text-[#003366] group-hover:text-blue-600 transition-colors leading-tight uppercase italic">{s.title}</h3>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-11 font-semibold px-2.5 py-1 rounded-full bg-blue-600/10 border border-blue-600/20 text-blue-600">Verified</span>
          </div>
        </div>

        {/* Quote */}
        {s.why && (
          <p className="text-sm text-[var(--text-secondary)] leading-relaxed italic border-l-2 border-blue-600/30 pl-3">
            "{s.why}"
          </p>
        )}
          ...
          >
            <div className="flex gap-2.5">
              ...
              <div className="min-w-0 space-y-1 flex-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-400">Quick check</p>
                <p className="text-xs text-amber-100/95 leading-snug">{locationCheckText}</p>
              </div>
            </div>
            {locationCheckAnswer === null ? (
              <div className="flex flex-wrap items-center gap-2 pl-7 sm:pl-0 sm:justify-end">
                <span className="text-[10px] font-semibold text-amber-200/80 uppercase tracking-wide w-full sm:w-auto sm:mr-auto">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-blue-600/90 hover:bg-blue-600 text-black text-xs font-bold transition-colors shadow-sm shadow-blue-600/20"
                >
                  Yes
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 border border-white/15 text-amber-50 text-xs font-bold transition-colors"
                >
                  No
                </button>
              </div>
                  ...
                  <tr key={idx} className={`border-b last:border-0 border-slate-200 transition-all duration-300 ${isGrey ? 'opacity-25 grayscale' : ''}`}>
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-2">
                        <item.icon className={`w-3.5 h-3.5 ${isGrey ? 'text-slate-500' : `text-${item.color}-400`}`} />
                        <div className="min-w-0">
                          <p className="text-[10px] font-black text-[#003366] uppercase tracking-tight leading-none">{item.label}</p>
                    ...
                    <td className="py-2.5 px-3 text-right">
                      <span className={`text-[11px] font-black italic tracking-tighter ${isGrey ? 'text-zinc-600' : 'text-[#003366]'}`}>
                        {isNoData ? '—' : formatPrice(item.val, item.pax)}
                      </span>
                    </td>
                  </tr>
                );
        ...
        <div className="grid grid-cols-3 gap-1.5">
          <div className="flex items-center gap-1.5 bg-blue-500/5 px-2 py-2 rounded-lg border border-blue-500/10">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-black text-[#003366] truncate uppercase italic">{s.safety_score || '9.5/10'}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 bg-blue-600/5 px-2 py-2 rounded-lg border border-blue-600/10">
            <Heart className="w-3.5 h-3.5 text-blue-600 shrink-0" />
            <div className="min-w-0">
              <p className="text-[8px] text-blue-600 font-black uppercase">Tip</p>
              <p className="text-[10px] font-black text-[#003366] truncate uppercase italic">{s.caring_tip || 'Carry light layers'}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 bg-blue-600/5 px-2 py-2 rounded-lg border border-blue-600/10">
            <Leaf className="w-3.5 h-3.5 text-blue-600 shrink-0" />
            <div className="min-w-0">
              <p className="text-[8px] text-blue-600 font-black uppercase">Eco</p>
              <p className="text-[10px] font-black text-[#003366] truncate uppercase italic">{s.sustainability_hint || 'Train available'}</p>
            </div>
          </div>
        </div>
          ...
          >
           ...
           {pending ? (
            <div className="flex items-center gap-1.5 ml-auto">
              <span className="text-[10px] font-black text-[#003366] uppercase hidden sm:block italic">Build trip?</span>
              <button
                onClick={() => { setPending(false); onConfirm(s, s.destination); }}
                className="flex items-center gap-1 px-4 py-2 rounded-lg bg-blue-600 text-black text-[10px] font-black uppercase transition-all shadow-lg shadow-blue-600/20 active:scale-95 italic"
              >
  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-2 pt-2 md:landscape:space-y-1 md:landscape:pt-0">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-600/10 border border-blue-600/20 mb-1">
          <Sparkles className="w-3.5 h-3.5 text-blue-600" />
          <span className="text-xs font-semibold text-blue-600">AI Discovery</span>
        </div>
        <h2 className="text-2xl font-semibold text-[#003366]">
          Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-orange-400">Options</span>
        </h2>
        <p className="text-caption">From {inputs.origin} · Budget ₹{inputs.targetBudget.toLocaleString()}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 py-3 md:landscape:py-2 border-y border-slate-200">
        ...
        {(['adults', 'kids'] as const).map(type => (
          <div key={type} className="flex items-center gap-3 bg-[var(--bg-surface)] rounded-xl px-4 py-2 border border-[var(--border)]">
            ...
            <button onClick={() => setInputs(p => ({ ...p, [type]: Math.max(type === 'adults' ? 1 : 0, (p as any)[type] - 1) }))} className="text-[var(--text-secondary)] hover:text-[#003366]">−</button>
            <span className="text-sm font-semibold text-blue-600 w-4 text-center">{(inputs as any)[type]}</span>
            <button onClick={() => setInputs(p => ({ ...p, [type]: (p as any)[type] + 1 }))} className="text-[var(--text-secondary)] hover:text-[#003366]">+</button>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-5">
        {suggestions.map((s, i) => (
          <div key={i} className={`relative rounded-2xl transition-all ${selectedIdx === i ? 'ring-2 ring-blue-600 ring-offset-4 ring-offset-[#020617]' : ''}`}>
            {selectedIdx === i && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-black text-[10px] font-black uppercase px-3 py-1 rounded-full z-10 shadow-lg italic">
                Current Selection
              </div>
            )}
            <SuggestionCard
      ...
      <div className="flex flex-col items-center gap-4 pt-4">
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-xs font-black text-zinc-950 uppercase tracking-widest bg-gradient-to-r from-cyan-500 via-white to-green-600 px-8 py-4 rounded-2xl shadow-lg shadow-cyan-500/10 transition-all active:scale-95"
        >
          ← {t('adjust_details')}
        </button>
        <button
          onClick={onReset}
          className="flex items-center gap-2 text-[10px] font-medium text-zinc-600 hover:text-[#003366] transition-colors"
        >
