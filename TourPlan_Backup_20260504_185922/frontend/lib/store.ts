...
let tripPlannerClearRef: (() => void) | null = null;
...
export const useTripStore = create<TripState>((set, get) => ({
  ...
  setDestination: (destination) => {
    ...
    // Clear any cached plan when destination changes — prevents stale AI context
    // We use a deferred call to avoid circular module dependency
    setTimeout(() => {
      tripPlannerClearRef?.();
    }, 0);
    set({ destination });
...
interface TripPlannerState {
  ...
  mixPicks: { transport: any; hotel: any; local: any };
...
export const useTripPlannerStore = create<TripPlannerState>()(
  persist(
    (set) => ({
      ...
      selectedPlan: null,
      hotelTier: null,
      mixPicks: { transport: null, hotel: null, local: null },
...
// Register the cross-store bridge after store is created
tripPlannerClearRef = () => useTripPlannerStore.getState().clearPlan();

// ────────────────────────────────────────────────────────────────────────────
// Tour Guide Store — single shared trip state for TourPlan ↔ Tour Guide sync
