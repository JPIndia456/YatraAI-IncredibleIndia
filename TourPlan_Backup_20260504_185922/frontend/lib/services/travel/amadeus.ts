...
export const AmadeusService = {
  ...
  async searchFlights(params: {
    ...
    try {
      ...
      // Transform Amadeus "Offers" into TourPlan "Flights" structure
      return (data.data || []).map((offer: any) => {
        const seg = offer.itineraries[0].segments[0];
        return {
          id: offer.id,
          airline: 'Partner Airline', // Need dictionary for carrier codes
   ...
   * Returns TourPlan-shaped hotel rows or null on hard failure.
   */
