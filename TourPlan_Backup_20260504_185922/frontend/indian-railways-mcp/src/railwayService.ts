import { parseLiveStationStatus, parseTrainData, parseBetweenStationsData, parseRouteData, parsePnrStatus } from "./railwayUtils";
  ...
  export async function getTrainsOnDate<T>(from: string, to: string, date: string): Promise<T | null> {
    ...
    try {
      ...
      json["data"].forEach((ele: any) => {
        if (ele["trainBase"]["operatingDays"][day] == '1') arr.push(ele);
      });
      retval["success"] = true;
      retval["time_stamp"] = Date.now();
      retval["data"] = arr;
