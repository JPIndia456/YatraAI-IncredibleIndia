...
/**
 * SysYatra Logger
 * Performance-optimized structured logger for the Yatra Ecosystem.
 */
export const SysLogger = {
  async log(service: string, message: string, level: LogLevel = 'INFO', metadata: any = {}) {
    ...
    try {
      await supabaseAdmin.from('yatra_sys_logs').insert([{
        service,
        level,
        message,
        metadata
  ...
  async claim(service: string, claimId: string): Promise<boolean> {
    try {
      const { error } = await supabaseAdmin.from('yatra_automation_claims').insert({
        claim_id: claimId,
        service
      });
      return !error;
