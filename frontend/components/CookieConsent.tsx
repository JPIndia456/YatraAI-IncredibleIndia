...
export default function CookieConsent() {
  ...
  useEffect(() => {
    ...
    const existing = localStorage.getItem('yatra_cookie_consent');
    if (!existing) {
  ...
  const handleConsent = (level: ConsentLevel) => {
    localStorage.setItem('yatra_cookie_consent', JSON.stringify({
      level,
      ...
      >
        <div className="bg-zinc-900/98 border border-zinc-700 rounded-[1.75rem] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.7)] backdrop-blur-3xl">
          ...
          <p className="text-xs text-zinc-400 mt-4 leading-relaxed">
            Yatra collects only the data necessary to provide our AI-powered travel services —
          ...
          <p className="text-[9px] text-zinc-700 text-center mt-3">
            By using Yatra you agree to our{' '}
