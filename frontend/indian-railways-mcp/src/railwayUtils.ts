...
export function parseLiveStationStatus(html: string): LiveStationStatusResponse {
  ...
  const tableMatch = html.match(/<table[^>]*class=["'][^"']*StationLive2[^"']*["'][\s\S]*?<\/table>/i);
  ...
  rowMatches.forEach((rowHtml) => {
    ...
    const timeMatch = rowHtml.match(
      /<[^>]*class=["'][^"']*\bbold\b[^"']*\bgreenLink\b[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/i
    );
    const trainName = nameMatch ? stripTags(nameMatch[1]) : "";
    const trainTime = timeMatch ? stripTags(timeMatch[1]) : "";

    if (trainName && trainTime) {
      trains.push({ name: trainName, time: trainTime });
    }
  });

  const stationMatch = html.match(/<h1[^>]*class=["'][^"']*mainpageH1[^"']*["'][^>]*>([\s\S]*?)<\/h1>/i);
  const station = stationMatch ? stripTags(stationMatch[1]) : "";
