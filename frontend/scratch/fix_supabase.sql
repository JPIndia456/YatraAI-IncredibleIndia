-- 1. FIX yatra_bookings
ALTER TABLE public.yatra_bookings RENAME COLUMN trip_data TO trip_details;
ALTER TABLE public.yatra_bookings RENAME COLUMN total_amount TO total_price;

-- 2. ENABLE RLS
ALTER TABLE public.yatra_bookings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.yatra_tour_guide_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.yatra_trip_plans ENABLE ROW LEVEL SECURITY;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own bookings') THEN
        CREATE POLICY "Users can manage their own bookings" ON public.yatra_bookings
        FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
    END IF;

    -- yatra_tour_guide_sessions
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own sessions') THEN
        CREATE POLICY "Users can manage their own sessions" ON public.yatra_tour_guide_sessions
        FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
    END IF;

    -- yatra_trip_plans
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own plans') THEN
        CREATE POLICY "Users can manage their own plans" ON public.yatra_trip_plans
        FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
    END IF;
