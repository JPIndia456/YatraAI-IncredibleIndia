...
export async function POST(req: Request) {
  try {
    ...
    const { data: booking, error: bookingError } = await supabaseAdmin
      .from('yatra_bookings')
      .select('id, user_id, status, trip_details')
      .eq('id', bookingId)
      .single();
    ...
    const nextTripData = {
      ...
      providerRef: providerRef || booking.trip_details?.providerRef || null,
    ...
    await supabaseAdmin.from('yatra_notifications').insert([
      {
